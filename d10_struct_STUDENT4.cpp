...
void QLSV() {
	...
	//b5: sap xep lai bang ds theo thu tu tang dan cua cot ten
	struct STUDENT temp;
	for(int i=0; i<n-1; i++){
		for(int k=i+1; k<n; k++){
	...
	//b6: in bang ds sinh vien sau khi sap xep thu tu theo cot ten
	printf("\n\n Danh sach sinh vien theo thu tu alphabeta \n");
	for(int i=0; i<n; i++) {
			printf("%2d. ", i+1);
			printf("%-10s ", ds[i].id );
			printf("%-30s ", ds[i].name );
